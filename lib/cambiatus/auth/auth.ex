defmodule Cambiatus.Auth do
  ...
  defp verify_signature(account, signature) do
    ...
    NodeJS.call({"app", :publicKeyPoints}, [signature, message])
    |> case do
  ...
  def verify_signature(phrase, signature, :test) do
    {:ok, phrase_encoded} = Jason.encode(phrase)

    # TODO implement using task
    NodeJS.call({"app", :publicKeyPoints}, [signature, phrase_encoded])
    |> case do
      {:ok, %{"publicKey" => pub_points, "signature" => signature_points}} ->
        public_key = %EllipticCurve.PublicKey.Data{
          curve: %EllipticCurve.Curve.Data{
        ...
        EllipticCurve.Ecdsa.verify?(phrase_encoded, signature_elixir, public_key)
