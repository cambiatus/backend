defmodule Cambiatus.Shop.Order do
  @moduledoc """
  This module holds the data structure that represents an instance of a `Cambiatus.Shop.Order` use it to
  build and validate changesets for operating on an order
  """
  use Ecto.Schema
  ...
  def order_has_items(changeset, order) do
    if Orders.has_items?(order) do
      ...
      add_error(changeset, :items, "Order has no items")
    end
  end
