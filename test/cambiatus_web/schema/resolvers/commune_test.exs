defmodule CambiatusWeb.Schema.Resolvers.CommuneTest do
  ...
  describe "Commune Resolver" do
    ...
    test "collects a community's features", %{conn: conn} do
      ...
      res = conn |> get("/api/graph", query: query)
      %{"data" => %{"community" => com}} = json_response(res, 200)

      assert com["features"]["actions"] == true
      assert com["features"]["shop"] == true
    end

    @tag :wip
    test "updates a community's features", %{conn: conn} do
      {:ok, %Community{} = community} =
        Commune.create_community(%{
          symbol: "TES",
          issuer: "testtesttest",
      ...
      %{"data" => %{"updateFeatures" => com}} = json_response(res, 200)

      assert com["actions"] == false
      assert com["shop"] == false
    end
  end
end
