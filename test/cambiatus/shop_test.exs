defmodule Cambiatus.ShopTest do
  ...
  describe "orders" do
    ...
    test "deleting an order deletes all items associated to it" do
      ...
      Shop.delete_order(order)

      assert Shop.get_order(order.id) == {:error, "No order exists with the id: #{order.id}"}
      assert Shop.get_item(item1.id) == {:error, "No item exists with the id: #{item1.id}"}
      assert Shop.get_item(item2.id) == {:error, "No item exists with the id: #{item2.id}"}
      assert Shop.get_item(item3.id) == {:error, "No item exists with the id: #{item3.id}"}
    end
    ...
    test "valid checkout" do
      ...
      assert {:ok, %{order | status: "checkout"}} ==
               Shop.update_order(order, %{status: "checkout"})
    end

    test "refute checkout without items" do
      order = insert(:order, %{items: [], status: "cart"})

      assert {:error, changeset} = Shop.update_order(order, %{status: "checkout"})
      assert [items: {"Order has no items", []}] == changeset.errors
    end
  ...
  describe "items" do
    ...
    test "create_item/1 with valid data creates an item", %{user: user} do
      assert {:ok, %Item{} = item} = Shop.create_item(Map.merge(@valid_attrs, %{buyer: user}))
    end

    test "create_item/1 with invalid data returns error changeset", %{user: user} do
      assert {:error, %Ecto.Changeset{}} =
               Shop.create_item(Map.merge(@invalid_attrs, %{buyer: user}))
    end
